...
class CampaignsAjax extends Controller {
    ...
    private function create() {
        ...
        if($this->user->plan_settings->campaigns_limit != -1 && $account_total_campaigns >= $this->user->plan_settings->campaigns_limit) {
            $errors[] = language()->create_campaign_modal->error_message->campaigns_limit;
        }
        ...
        if(empty($errors)) {
            ...
            Response::json(language()->create_campaign_modal->success_message, 'success', ['campaign_id' => $campaign_id]);
    ...
    private function update() {
        ...
        if(empty($errors)) {
            ...
            Response::json(language()->update_campaign_modal->success_message, 'success');
